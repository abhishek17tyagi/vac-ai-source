...
def main(argv: list[str]) -> None:
    ...
    # Variables for VacAI from .env
    initial_states_path = (
        FLAGS.initial_states_path
        if FLAGS.initial_states_path
        else os.getenv("VacAI_SCENARIO")
    )
    env_vars["VacAI_SCENARIO"] = initial_states_path
    map_key = (
        FLAGS.initial_states_path
        if FLAGS.initial_states_path
    ...
    if not project_id:
        ...
        print("Missing required environment variable: VacAI_SCENARIO")
        return
    elif not map_key:
